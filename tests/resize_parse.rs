...
fn parse_resize_both() {
    assert_eq!(parse_resize_spec("1920x1080").unwrap(), ResizeSpec {
        w: Some(1920),
        h: Some(1080),
    });
}
...
fn parse_resize_width_only() {
    assert_eq!(parse_resize_spec("1920x").unwrap(), ResizeSpec { w: Some(1920), h: None });
}

#[test]
fn parse_resize_height_only() {
    assert_eq!(parse_resize_spec("x1080").unwrap(), ResizeSpec { w: None, h: Some(1080) });
}

#[test]
fn parse_resize_trims() {
    assert_eq!(parse_resize_spec("  800x600 ").unwrap(), ResizeSpec { w: Some(800), h: Some(600) });
}
